...
public class SnapshotRoot extends AbstractSnapshot<byte[], byte[]> {
  ...
  private static final Set<String> PRUNE_DBS = Arrays.asList( "block-index","block", "trans",
      "transactionRetStore").stream().collect(Collectors.toSet());
  ...
  public SnapshotRoot(DB<byte[], byte[]> db) {
    ...
    if (PRUNE_DBS.contains(this.db.getDbName())) {
